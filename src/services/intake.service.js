...
const generateIntakeEventsForRegimen = async (regimen, transaction = null) => {
  try {
    ...
    // Parse DATEONLY an toàn theo timezone (tránh lệch ngày khi runtime ở UTC)
    const startRaw = String(start_date || "");
    const endRaw = String(end_date || "");

    const startDT = /^\d{4}-\d{2}-\d{2}$/.test(startRaw)
      ? DateTime.fromISO(startRaw, { zone: tz })
      : DateTime.fromJSDate(new Date(start_date), { zone: tz });

    const endDT = /^\d{4}-\d{2}-\d{2}$/.test(endRaw)
      ? DateTime.fromISO(endRaw, { zone: tz })
      : DateTime.fromJSDate(new Date(end_date), { zone: tz });
