...
const {
  assertProfileRole,
  getProfileAccess,
} = require("./profile-access.service");
...
// Roles
const CAN_READ = ["owner", "caregiver", "viewer"];
const CAN_WRITE = ["owner", "caregiver"]; // viewer read-only

const createPrescription = async (userId, profileId, data) => {
  // must have write access to create
  await assertProfileRole(userId, profileId, CAN_WRITE);
...
const addPrescriptionItem = async (userId, prescriptionId, data) => {
  ...
  await assertProfileRole(userId, prescription.profile_id, CAN_WRITE);
...
const getPrescriptionById = async (userId, prescriptionId) => {
  ...
  await assertProfileRole(userId, prescription.profile_id, CAN_READ);
...
// FE Phase 1 uses /prescription-items/:itemId (without prescriptionId)
const updatePrescriptionItemById = async (userId, itemId, data) => {
  const item = await PrescriptionItem.findByPk(itemId);
  if (!item) throw httpError("Không tìm thấy thuốc trong đơn", 404);

  const prescription = await Prescription.findByPk(item.prescription_id);
  if (!prescription) throw httpError("Không tìm thấy đơn thuốc", 404);

  return await updatePrescriptionItem(userId, prescription.id, itemId, data);
};
...
const deletePrescriptionItemById = async (userId, itemId) => {
  const item = await PrescriptionItem.findByPk(itemId);
  if (!item) throw httpError("Không tìm thấy thuốc trong đơn", 404);
  ...
  await deletePrescriptionItem(userId, prescription.id, itemId);
};

const deletePrescriptionFileById = async (userId, fileId) => {
  const file = await PrescriptionFile.findByPk(fileId);
  if (!file) throw httpError("Không tìm thấy file", 404);

  const prescription = await Prescription.findByPk(file.prescription_id);
  if (!prescription) throw httpError("Không tìm thấy đơn thuốc", 404);

  await assertProfileRole(userId, prescription.profile_id, CAN_WRITE);

  // Phase 2: only delete DB record (no storage integration)
  await file.destroy();
};
...
const listPrescriptionsByProfile = async (
  ...
  await assertProfileRole(userId, profileId, CAN_READ);
